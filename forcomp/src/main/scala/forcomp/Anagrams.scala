...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy((element: Char) => element).toList.map(x => (x._1, x._2.length)).sortWith(_._1 < _._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case Nil => List(Nil)
    case head :: tail => for {
      newHead <- (0 until head._2 + 1).map(i => (head._1, i)).toList
      newTail <- combinations(tail)
    } yield (newHead :: newTail).filter(x => x._2 != 0)
  }
